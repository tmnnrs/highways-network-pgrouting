<?php
  ...
  $sql = "SELECT a.*, b.length, ST_AsGeoJSON(ST_Transform(b.geom, 4326), 7) AS geom
          FROM pgr_astar(
            'SELECT id, source, target, cost_$option$period AS cost, reverse_cost_$option$period AS reverse_cost, x1, y1, x2, y2
  ...
  $_cost = gmdate('H:i:s', floor($_cost * 60));

  if( $option != 'distance' ) {
    $geojson['metadata']['time'] = $_cost;
  }
